...
static esp_err_t device_info_handler(httpd_req_t *req) {
  ...
  cJSON_AddNumberToObject(json, "led_count", CONFIG_LED_ANIM_COUNT);
  cJSON_AddStringToObject(json, "color_order", "GRB");
  cJSON_AddStringToObject(json, "rgb_type", "RGB8");

  const esp_app_desc_t *app_desc = esp_app_get_description();
  cJSON_AddStringToObject(json, "firmware_version", app_desc->version);
...
esp_err_t web_server_start(uint16_t port) {
  ...
  httpd_uri_t device_info_uri = {.uri = "/api/device/info",
                                 .method = HTTP_GET,
                                 .handler = device_info_handler};
  httpd_register_uri_handler(s_server, &device_info_uri);
